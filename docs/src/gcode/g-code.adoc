...
* 'R' - Retract distance after each pass. Defaults to 0.5, or to the value
  stored by a previous parameter line.
* 'U' (execute line) - X finishing allowance as a *diameter* value.
  Positive for OD turning, negative for ID/boring.
...
and G3. Those blocks are a shape definition: after G71.3 or G70.3 they
are skipped so they are not cut as ordinary motion. G70.3 later traces
the same `P`/`Q` range as a finishing pass.

Type I vs Type II is taken from the first motion in the `P` block, as on
Fanuc:

* Type I (no pockets) - the `P` block moves in X only.
...
* The command is given from MDI (there is no file to search for `N`
  numbers).
* The profile is not monotonic in Z, the plane is not ZX (`G18`), or
