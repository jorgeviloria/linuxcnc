...
|<<gcode:g10-l12,G10 L12>>         |Set Tool Wear
...
|<<gcode:g70.3,G70.3>>             |Fanuc-style lathe finishing cycle
|<<gcode:g71-g72,G71-G72>>         |Lathe roughing cycle
|<<gcode:g71.3,G71.3>>             |Fanuc-style lathe roughing cycle
...
|<<gcode:g90-g91,G90 G91>>         |Distance Mode (<<gcode:fanuc-lathe,Fanuc lathe system A>>: G90 is the turning cycle)
|<<gcode:g90.1-g91.1,G90.1 G91.1>> |Arc Distance Mode
|<<gcode:g92,G92>>                 |Coordinate System Offset (<<gcode:fanuc-lathe,Fanuc lathe system A>>: G92 is the threading cycle)
|<<gcode:g92.1-g92.2,G92.1 G92.2>> |Cancel G92 Offsets
|<<gcode:g92.3,G92.3>>             |Restore G92 Offsets
|<<gcode:g93-g94-g95,G93 G94 G95>> |Feed Modes (<<gcode:fanuc-lathe,Fanuc lathe system A>>: G94 is the facing cycle, G98/G99 are the feed modes)
...
[[gcode:g10-l12]]
== G10 L12 Set Tool Wear(((G10 L12 Tool Wear)))

[source,ngc]
----
G10 L12 P- <X- Y- Z- A- B- C- U- V- W- R->
----

* 'P' - tool / offset number whose wear is set
* 'X' 'Z' (and other axes) - absolute wear, in the same units as the
  geometry offsets on that tool
* 'U' 'W' (and 'V') - incremental wear, added to the stored wear
* 'R' - wear diameter (same convention as `G10 L1 R`: a radius, stored
  as diameter in `WD`, not applied to motion)

G10 L12 writes the native wear fields (`WX` `WY` `WZ` ... `WD` in the
tool table).  It does not change geometry. `X` and `U` cannot appear
together (same for `Z`/`W` and `Y`/`V`).

.G10 L12 Example
[source,ngc]
----
G10 L12 P1 X-0.02 Z0.01    (set wear of offset 1)
G10 L12 P1 U-0.01          (add -0.01 to X wear)
----

Used with <<sec:select-tool,T: Select Tool>> when `LATHE_TXXXX` is
enabled: `T0101` applies tool 1 geometry plus this wear. Without
`LATHE_TXXXX`, the values are stored and still written to the tool table.

It is an error if:

* Cutter compensation is on
* The P number is unspecified or not a valid tool number
* No wear word is given
* Both an absolute and an incremental word for the same axis are given
...
identity kinematics, where joints and coordinates are the same thing.
Which number that is is declared by the module itself, so 'G13.1' means
the same kinematics whatever order a module lists its types in.
...
which one is identity. See the `switchkins` section of the kins(9) man
page for the modules that support switching and the order in which they
...
module that declares no identity kinematics; such a module can still be
driven by number with 'G12.1'.
...
machine in identity kinematics.
...
* 'G13.1' is used and the module declares no identity kinematics.
...
For a Fanuc-style finishing pass over `N` sequence numbers, see
<<gcode:g70.3,G70.3>>.
...
[NOTE]
G71/G72/G71.3 rough with **constant-X scan passes** (Fanuc-style): each
depth of cut is a finite set of Z intervals, so a profile with pockets
can no longer make the cycle hang. A `D` offset will not insert a huge
connecting arc (see issue
https://github.com/LinuxCNC/linuxcnc/issues/2939[#2939]). The profile
must still be monotonic in Z (G71) or X (G72).

After the scan passes, G71 and G72 (without suffix) follow the offset
profile once with a finishing move, so tapers, radii and shoulders are
left with a uniform finishing allowance instead of steps of one depth
of cut. G7x.1 and G7x.2 do not follow the profile.
...
The G71 and G72 cycles (without suffix) cut the outer envelope and the
pockets before following the profile; G7x.1 only cuts the envelope and
G7x.2 only cuts the pockets. Every scan pass returns to the starting X.

. Preliminary motion.
  ** If Z or X are used a <<gcode:g0,rapid move>> to that position
     is done.
  ** After the scan passes, G71 and G72 follow the profile and the tool
     stops at its end, including the distance specified in D. G7x.1 and
     G7x.2 stop at the starting X instead.
. The D number is used to  keep a distance from the final profile,
  to allow material to remain for finishing.
. A finishing pass (<<gcode:g70,G70>> or a second cycle with a smaller
  D) is still needed to remove the material left by D.
...
[[gcode:g71.3]]
[[gcode:g70.3]]
== G71.3 G70.3 Fanuc-style lathe cycles(((G71.3 G70.3 Fanuc-style lathe cycles)))

These cycles follow the two-line Fanuc 0i/30i G71/G70 dialect. The
existing <<gcode:g71-g72,G71>> / <<gcode:g70,G70>> cycles (subroutine `Q`)
share the same **scan-pass** engine, but G71/G72 also follow the offset
profile once at the end. G71.3 roughing is scan passes only:
Type I (X-only first block) cuts only the outer envelope at each
depth; Type II also cuts pocket intervals. Finishing allowance is
`U`/`W` translation, not a `D` curve-offset, and a finishing pass
(G70.3, or G70 with a subroutine) is needed to finish the profile.
`G70.3` with the same `P`/`Q` numbers reuses the range `G71.3` found;
otherwise the range is searched after the cycle first and then from the
start of the program, as Fanuc does. Avoid reusing `N` numbers that
appear before the profile.

[source,ngc]
----
G71.3 U- R-                          (parameter line, optional)
G71.3 P- Q- <U-> <W-> <I-> <R-> <F->  (execute line)
N(ns) ... profile ... N(nf)
G70.3 P- Q-
----

One-line form (depth of cut on the execute line):
...
* 'P' - Sequence number (`N`) of the first block of the finished profile.
* 'Q' - Sequence number (`N`) of the last block of the finished profile.
* 'U' (parameter line) - Depth of cut per roughing pass, as a *radius* value
  (same as `I` on G71).
* 'R' - Retract distance after each pass. Defaults to 0.5 mm or 0.020 inch,
  or to the value stored by a previous parameter line.
* 'U' (execute line) - X finishing allowance, in the same units as X:
  *diameter* in G7, *radius* in G8. Positive for OD turning, negative
  for ID/boring.
* 'W' - Z finishing allowance.
* 'I' - Depth of cut (radius). Used when there is no parameter-line `U`,
  or to override it.

`U` and `W` on the G71.3/G70.3 line are cycle words, not extra-axis
moves. Write `G` before `U`/`W` on that line (Fanuc order), e.g.
`G71.3 U2 R0.5`. They are accepted even if the machine has no U/W axes.

The profile is written inline between `N(P)` and `N(Q)` with G0, G1, G2
and G3. Put this range *after* the G71.3 (or G70.3) command. Those
blocks are a shape definition: after the cycle they are skipped so they
are not cut as ordinary motion.

If the current motion mode is not already G0, G1, G2 or G3 (for example
after G80), the first profile block must contain G0, G1, G2 or G3. A
bare `X`/`Z` move is allowed when G0–G3 is already modal.

Parameter assignments (`#`) between the G71.3 line and `N(P)` are used
when the profile is read, then those lines still run once after the
cycle. Assignments inside `P`–`Q` are used only while that cycle reads
the profile, then discarded: skipped lines do not store them. Do not
put mutable `#` inside `P`–`Q` (for example `#100=[#100+1]`); each
G71.3/G70.3 re-reads the range with the live values.

G70.3 later traces the same `P`/`Q` range as a finishing pass. Feed for
that pass is: `F` on the G70.3 line if given, otherwise the first `F` in
the P-Q profile, otherwise the modal feed. G71.3 ignores `F` in the
profile (Fanuc: roughing feed comes from the G71.3 line).

Type I vs Type II is taken from the first motion in the `P` block, as on
Fanuc:

* Type I (no pockets) - the `P` block moves in X only. X from A' to B
  must be monotonic (Fanuc alarm if it is not).
* Type II (pockets allowed) - the `P` block includes X and Z (use `W0`
  if Z must not move).

The cycle start point is the tool position before G71.3 (optional `X`/`Z`
on the G71.3 line rapids there first). After roughing or finishing the
tool returns to that start point.

It is an error if:

* `P` or `Q` is missing on the execute line.
* No `N(P)` or `N(Q)` exists in the current program file.
* Depth of cut is missing (`U` on the parameter line or `I` on the
  execute line).
* The command is given from MDI (there is no file to search for `N`
  numbers).
* The profile is not monotonic in Z, a Type I profile is not monotonic
  in X, the plane is not ZX (`G18`), or cutter compensation is active.
...
[[gcode:fanuc-lathe]]
== Fanuc Lathe G-code System A (((Fanuc Lathe G-code System A)))

With `[RS274NGC]FANUC_LATHE = 1` the interpreter accepts the codes that
Fanuc lathes use in G-code system A:

[width="80%",align="center",options="header"]
|===
|Code |LinuxCNC default |With FANUC_LATHE

|G50  |not defined      |spindle speed clamp (S) and coordinate system setting (X Z)
|G90  |distance mode    |turning cycle with axis words; without them, absolute mode
|G92  |coordinate offset|threading cycle; use G50 for the coordinate offset
|G94  |feed per minute  |facing cycle with axis words; without them, feed per minute
|G98  |cycle retract mode|feed per minute (like LinuxCNC G94)
|G99  |cycle retract mode|feed per revolution (like LinuxCNC G95)
|===

All other codes keep their LinuxCNC meaning.  The mode is off by
default; with it off these codes behave exactly as documented elsewhere
in this manual.

=== G90 Outer/Inner Diameter Turning Cycle
...
* 'X' 'Z' - end point of the pass (diameter and length)
* 'U' 'W' - incremental end point, as in the G7x profiles. 'U' is a
  diameter when G7 lathe diameter mode is active.
* 'R' - taper amount, a radius value: the cut starts at 'X - 2R' and
  ends at 'X'.  For a diameter that decreases towards the chuck (Z-), R
  is negative, as on the control.
* 'F' - cutting feed

The cycle runs:
. rapid to the start of the cut,
. cutting feed to the end point,
. cutting feed back along the shoulder to the start diameter,
. rapid back to the cycle start point.

The tool ends at the cycle start point, so a block with only X/Z/U/W
repeats the cycle.  The programmed X/Z words stay modal for the cycle,
like on the control.

.G90 example, several passes
[source,ngc]
----
G0 X61 Z2
G90 X55 W-42 F0.25
...
Same shape as G90 with X and Z exchanged:
. rapid in Z,
. cutting feed in X,
. cutting feed back in Z,
. rapid back to the cycle start point.

G94 has no taper word.

.G94 example, facing in steps
[source,ngc]
----
G0 X55 Z2
G94 X15 Z-2 F0.2
...
One spindle synchronized pass per block; repeat the block with a new X
to cut the next pass.  'F' is the thread lead, in program units per
spindle revolution.  The spindle must be turning before the cycle
starts, and there is no cutter radius compensation.

.G92 example, external thread in passes
[source,ngc]
----
G0 X1.1 Z0.2
M3 S500
G92 X0.98 Z-0.8 F0.05
X0.96
X0.94
----

G92 on the 0i-TF is straight threading only (format `X Z F Q`); the
optional 'Q' thread chamfering word and the older taper forms are not
supported yet and are rejected with an error.  Use <<gcode:g76,G76>> for
tapered threads.

=== G50 Spindle Speed Clamp and Coordinate System
...
'G50 S' sets the maximum spindle speed used by constant surface speed,
the same value as the 'D' word of <<gcode:g96-g97,G96>>.  The clamp is
remembered when G96 is programmed without a D word.

'G50 X Z' sets the coordinate system so the current tool position reads
as the given values, exactly like the coordinate offset that LinuxCNC
normally programs with G92.  Cancel it with G92.1/G92.2.
...
G96 S180 M3
G0 X61 Z2
G50 X60 Z0      (the tool at this position now reads X60 Z0)
----

=== Limits

* The cycles always work in the XZ plane.
* Cutter radius compensation must be off.
* U/W are incremental X/Z; they cannot be combined with X/Z in one
  block, nor used in incremental distance mode (G91).
* Every cycle ends at the cycle start point, which is what makes the
  modal repeat work and matches the control.
